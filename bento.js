...
function main() {
	...
	const generateBentoLinkButton = document.getElementById(
		'generate-bento-link'
	);
	...
	generateBentoLinkButton.addEventListener('click', (event) => {
		...
		const bentoData = [
			{
				...
				bentoDescription:
					document.getElementById('bento-description').value,
			},
		];
		...
		generatedLink.innerText = `${location.protocol}${
			location.host
		}/link.html?data=${window.btoa(
			encodeURIComponent(JSON.stringify(bentoData))
		)}`;
	});
